...
const API_URL_KEY = "dbd_rider_api_url";
const WS_URL_KEY = "dbd_rider_ws_url";
...
/**
 * Асинхронная инициализация. Должна вызываться на старте приложения
 * (в ApolloProviderClient) ДО создания Apollo-клиента.
 */
export async function initApiConfig(): Promise<ApiConfig> {
  // 1) AsyncStorage override — самый высокий приоритет
...
/**
 * Синхронные геттеры. До initApiConfig() вернут безопасный fallback.
 * Использовать ТОЛЬКО в уже инициализированном дереве компонентов.
 */
...
/**
 * Установить override. Сохраняет в AsyncStorage, обновляет кэш.
 * После вызова нужно пересоздать Apollo-клиент (см. ApolloProviderClient).
 */
...
export async function clearApiUrlOverride(): Promise<void> {
  ...
  // Переинициализируем с дефолтами
  _resolvedHttp = null;
  _resolvedWs = null;
