...
function getToken(): string | undefined {
  return (globalThis as any).__DBD_RIDER_TOKEN__ as string | undefined;
}
  ...
  const errorLink = onError((options: any) => {
    ...
    if (graphQLErrors) {
      for (const err of graphQLErrors) {
        ...
        if (err.extensions?.code === "RIDER_UNAVAILABLE") continue;
        console.warn(
...
/** Сброс singleton — вызываем при смене URL через override */
...
/**
 * Singleton accessor — позволяет дёргать Apollo-клиент из любого места
 * (например, в chat/[orderId].tsx вне React-дерева после setState).
 * Возвращает текущий клиент или null, если ещё не инициализирован.
 */
...
export function ApolloProviderClient({ children }: { children: ReactNode }) {
  ...
  // FIX: в Apollo Client v4 + React 19 необходимо явно передать children={undefined},
  // чтобы TypeScript корректно вывел JSX с детьми
  return (
