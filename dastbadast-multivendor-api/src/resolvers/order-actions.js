...
function requireRestaurant(ctx) {
  if (!ctx.restaurant) throw new GraphQLError('Not authenticated', { extensions: { code: 'UNAUTHENTICATED' } });
...
export const acceptOrder = async (_p, { input }, ctx) => {
  ...
  if (!order) throw new GraphQLError('Заказ не найден', { extensions: { code: 'NOT_FOUND' } });
  if (order.restaurantId.toString() !== r._id.toString()) {
    throw new GraphQLError('Forbidden', { extensions: { code: 'FORBIDDEN' } });
  }
  if (!ALLOWED_FROM.ACCEPTED.includes(order.orderStatus)) {
    throw new GraphQLError(`Невозможно принять заказ в статусе ${order.orderStatus}`, {
  ...
  pubsub.publish(TOPICS.AVAILABLE_ORDERS(order.zoneId?.toString()), { subscriptionAvailableOrders: order });
  return order;
};
...
export const cancelOrder = async (_p, { input }, ctx) => {
  ...
  if (!ALLOWED_FROM.CANCELLED.includes(order.orderStatus)) {
    throw new GraphQLError(`Невозможно отменить заказ в статусе ${order.orderStatus}`, {
      extensions: { code: 'BAD_STATE' },
    });
  }
